...
- **Krylov time evolution**: Matrix-free stroboscopic evolution via `scipy.sparse.linalg.expm_multiply` with adaptive subdivision (each half-period generator $A_\pm = -iH_\pm T/2$ is subdivided into $K_\pm = \[\frac{|A_\pm|^1</sub>}{5}\]$ substeps, where $\|A_\pm\|^1$ denotes the matrix 1-norm)
- **Diagnostics**:
